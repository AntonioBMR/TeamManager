...
public class AdaptadorJugadores extends ArrayAdapter<Jugador> {
    ...
    public AdaptadorJugadores(Context context, int resource, ArrayList<Jugador> objects) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        if (convertView == null) {
            ...
            vh = new ViewHolder();
            vh.tv = (TextView) convertView.findViewById(R.id.tvNombrej);
            convertView.setTag(vh);
        } else {
            vh = (ViewHolder) convertView.getTag();
        }
        vh.posicion = position;
        vh.tv.setText(lista.get(position).getNombrej().toString());
        return convertView;
    }
